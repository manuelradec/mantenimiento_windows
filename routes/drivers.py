...
@drivers_bp.route("/api/backup", methods=["POST"])
def api_backup_drivers():
    """M5 — export installed third-party drivers via pnputil /export-driver."""
    data = request.get_json(silent=True) or {}
    dest_path = data.get("dest_path", "")

    def handler():
        return drv_svc.backup_drivers(dest_path)
