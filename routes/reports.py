...
def _maint_type_label(maint_type):
    return _MAINT_TYPE_LABELS.get(
        (maint_type or "preventivo").lower(), "MANTENIMIENTO PREVENTIVO"
    )
...
def api_export_json():
    ...
    filepath = os.path.join(Config.REPORT_DIR, f'incident_{bundle["session_id"]}.json')
...
def api_export_txt():
    bundle = _build_incident_bundle()
    filepath = os.path.join(Config.REPORT_DIR, f'incident_{bundle["session_id"]}.txt')
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with open(filepath, "w", encoding="utf-8") as f:
...
def api_export_html():
    bundle = _build_incident_bundle()
    filepath = os.path.join(Config.REPORT_DIR, f'incident_{bundle["session_id"]}.html')
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with open(filepath, "w", encoding="utf-8") as f:
...
def _build_incident_bundle() -> dict:
    ...
    warnings = [
        e for e in audit_entries if e.get("status") in ("warning", "partial_success")
    ]
    ...
    return {
        ...
        "recommendations": _generate_recommendations(
            audit_entries, jobs, reboot_required
        ),
    }
        ...
        steps.append(
            {
                ...
                "message": entry.get("stdout_preview", "")
                or entry.get("stderr_preview", "")
                or "",
            }
        )
...
def api_export_fo_ti_20():
    ...
    filename = (
        f'FO-TI-20_{sucursal or "SUCURSAL"}_{datetime.now().strftime("%Y-%m-%d")}.html'
    )
    filepath = os.path.join(Config.REPORT_DIR, filename)
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with open(filepath, "w", encoding="utf-8") as f:
...
# ============================================================
# T-04 — Histórico consolidado a Excel
# ============================================================


@reports_bp.route("/api/download/historico-xlsx")
def api_download_historico_xlsx():
    """Genera y descarga el histórico consolidado de mantenimientos como .xlsx.

    Query params opcionales:
    - date_from: 'YYYY-MM-DD' (incluido).
    - date_to: 'YYYY-MM-DD' (incluido).
    - hostname: filtra Sessions/Maintenance Steps/Jobs por hostname exacto.

    Una sola request: genera el .xlsx desde SQLite y lo stremea como
    download. El archivo queda persistido en Config.REPORT_DIR para
...
def _generate_recommendations(entries, jobs, reboot_required) -> list[str]:
    ...
    if reboot_required:
        recs.append(
            "REBOOT REQUIRED: One or more actions require a system restart to complete."
        )

    error_count = sum(1 for e in entries if e.get("status") in ("failed", "error"))
    if error_count > 0:
        recs.append(
            f"{error_count} action(s) failed. Review errors and retry or escalate."
        )

    partial = sum(1 for e in entries if e.get("status") == "partial_success")
    if partial > 0:
        recs.append(
            f"{partial} action(s) partially succeeded. Review individual step results."
        )
...
def _render_text_report(bundle: dict) -> str:
    ...
    for entry in bundle.get("audit_entries", []):
        lines.append(
            f'  [{entry.get("timestamp", "")}] [{entry.get("module", "")}] '
...
def _render_html_report(bundle: dict) -> str:
    ...
    for entry in bundle.get("audit_entries", []):
        ...
        detail = (entry.get("stdout_preview") or entry.get("stderr_preview") or "-")[
            :150
        ]
