...
def _register_all_actions():
    ...
    for aid, name, module, desc, timeout in [
        ...
        (
            ...
            "Set Defender CPU Load",
            "security",
            "Adjust Defender scan CPU usage",
            30,
        ),
        (
            "security.quick_scan",
        ...
        (
            ...
            "Open SAC Settings",
            "security",
            "Open Windows Security Smart App Control page",
            10,
        ),
    ]:
        registry.register(
    ...
    for aid, name, module, desc, timeout, confirm_msg in [
        ...
        (
            ...
            "cleanup",
            "Remove obsolete Windows components",
            600,
            "",
        ),
        (
            ...
            "Release IP",
            "network",
            "Release current DHCP address (brief disconnect)",
            30,
        ...
        (
            ...
            "Clear SMB Sessions",
            "network",
            "Remove mapped drives and NetBIOS cache",
            30,
        ...
        (
            ...
            "repair",
            "Remove obsolete Windows components via repair module",
            600,
            "",
        ),
        (
        ...
        (
            ...
            "Programar Reinicio",
            "scheduled_restart",
            "Programa un reinicio del equipo vía Windows Task Scheduler",
            30,
            "Esto programará un reinicio del equipo. Continúe?",
        ),
        (
            "scheduled_restart.delete",
            "Cancelar Reinicio Programado",
            "scheduled_restart",
            "Cancela la tarea programada de reinicio",
            15,
            "Se eliminará la tarea programada existente. Continúe?",
        ),
    ]:
        registry.register(
            ActionDef(
    ...
    for aid, name, module, desc, timeout, confirm_msg, restore_pt, reboot, long_run in [
        (
            ...
            900,
            "SFC scan takes 5-15 minutes. Continue?",
            False,
            False,
            True,
        ),
        (
            ...
            "DISM CheckHealth",
            "repair",
            "Quick component integrity check",
            120,
            "",
            False,
            False,
        ...
        (
            ...
            "Deep component scan",
            600,
            "Deep scan may take 10+ minutes. Continue?",
            False,
            False,
            True,
        ),
        (
            ...
            "DISM RestoreHealth",
            "repair",
            "Repair component store from Windows Update",
            1800,
            "RestoreHealth may take 30+ minutes and download files. Continue?",
            True,
            False,
            True,
        ),
        (
            ...
            "CHKDSK Online Scan",
            "repair",
            "Online disk check without reboot",
            600,
            "Online CHKDSK scan may take several minutes. Continue?",
            False,
            False,
            True,
        ),
        (
            "repair.winsat",
            "WinSAT Disk Benchmark",
            "repair",
            "Windows System Assessment Tool for disk",
            120,
            "",
            False,
            False,
        ...
        (
            ...
            7200,
            "A full scan can take 2+ hours and will use significant CPU. Continue?",
            False,
            False,
            True,
        ),
        (
            ...
            600,
            "May require a reboot. Save all work. Continue?",
            True,
            True,
            True,
        ),
        ...
        (
            ...
            10,
            "This increases power consumption and fan speed. Continue?",
            False,
            False,
            False,
        ),
        ...
        (
            "network.repair",
            "Network Repair Sequence",
            "network",
            "Flush DNS + Release/Renew IP + Autotuning",
            60,
            "Brief disconnection possible during network repair. Continue?",
            False,
            False,
            False,
        ),
        (
            ...
            "Cambiar inicio de servicio de red",
            "network",
            ...
            "Puede afectar la conectividad de red. ¿Continuar?",
            False,
            False,
            False,
        ),
        (
            ...
            "network",
            "Habilitar un adaptador de red desactivado",
            30,
        ...
        (
            ...
            "network",
            "Deshabilitar un adaptador de red activo",
            30,
    ...
    for aid, name, module, desc, timeout, confirm_msg, reboot in [
        (
            "update.hard_reset",
            "Hard Reset Windows Update",
            "update",
            "Stop WU services and rename SoftwareDistribution/catroot2",
            120,
            "WARNING: This is the nuclear option for broken Windows Update. "
            "Only use when all other methods have failed. Continue?",
            True,
        ),
        (
            ...
            "Reset TCP/IP Stack",
            "network",
            "Reset entire TCP/IP networking stack",
            30,
            "WARNING: This resets the TCP/IP stack. You may lose connectivity. "
            "A reboot is REQUIRED afterward. Continue?",
            True,
        ),
        (
        ...
        (
            ...
            "Schedule Full CHKDSK",
            "repair",
            "Schedule full disk check on next reboot",
            600,
            "This schedules a full CHKDSK on next reboot (1-3 hours). Continue?",
            True,
        ),
        (
            ...
            "Memory Diagnostic",
            "repair",
            "Schedule memory diagnostic test on next reboot",
            10,
            "The memory test runs on next reboot. Continue?",
            True,
        ),
        (
            ...
            "Disable Hibernation",
            "power",
            "Delete hiberfil.sys and disable hibernate",
            10,
            "This deletes hiberfil.sys and frees space equal to your RAM. "
            "You will no longer be able to hibernate. Continue?",
            False,
        ),
        (
            ...
            "security",
            "Disable Windows Smart App Control (irreversible without clean install)",
            30,
            ...
            "reinstalando Windows 11 desde cero. ¿Desea continuar?",
            True,
        ),
    ]:
        registry.register(
...
def _register_office_actions():
    ...
    for aid, name, desc, timeout in [
        (
            "office.safe_mode",
            "Outlook modo seguro",
            "Iniciar Outlook con /safe (sin complementos)",
            10,
        ),
        (
            "office.configure_mail",
            "Configurar correo",
            "Abrir panel de perfiles de Outlook (mlcfg32.cpl)",
            10,
        ),
        (
            "office.scanpst",
            "Reparar PST (SCANPST)",
            "Iniciar herramienta de reparacion de archivos PST/OST",
            10,
        ),
    ]:
        registry.register(
    ...
    for aid, name, desc, timeout, confirm_msg, needs_admin in [
        (
            "office.repair_quick",
            "Reparacion rapida Office",
            "Ejecutar reparacion rapida de Office (ClickToRun, archivos locales)",
            300,
            "La reparacion rapida corregira archivos locales de Office. "
            "Office debe estar cerrado. Continuar?",
            True,
        ),
        (
            "office.repair_online",
            "Reparacion en linea Office",
            "Ejecutar reparacion en linea de Office (descarga todos los archivos)",
            1800,
            "La reparacion en linea descarga todos los archivos de Office desde Microsoft. "
            "Requiere internet y puede tardar 30+ minutos. Office debe estar cerrado. Continuar?",
            True,
        ),
        (
            "office.rebuild_index",
            "Reconstruir indice de busqueda",
            "Detener Windows Search, borrar indice y reconstruir (afecta toda la busqueda)",
            60,
            "Se detendra temporalmente Windows Search y se eliminara el indice actual. "
            "Afecta toda la busqueda del sistema (no solo Outlook). Continuar?",
            True,
        ),
    ]:
        registry.register(
