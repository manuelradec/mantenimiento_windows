...
from core.persistence import SnapshotStore
...
ROLLBACK_STRATEGIES = {
    ...
    "cleanup.delete_broken_shortcuts": {
        ...
        "reversible": "manual",
        "instructions": "Shortcuts go to Recycle Bin — restore from there if needed.",
        "needs_reboot": False,
        "restore_point_recommended": False,
    },
    ...
    "cleanup.delete_empty_folders": {
        "classification": "manually_reversible",
        "reversible": "manual",
        "instructions": "Empty folders go to Recycle Bin — restore from there if needed.",
        "needs_reboot": False,
        "restore_point_recommended": False,
    },
    "cleanup.delete_duplicate": {
        "classification": "manually_reversible",
        "reversible": "manual",
        "instructions": "Duplicate file goes to Recycle Bin — restore from there if needed.",
        "needs_reboot": False,
        "restore_point_recommended": False,
    },
    "drivers.backup": {
        "classification": "not_applicable",
        "reversible": "n/a",
        "instructions": "Backup creates new files; nothing to reverse. To restore drivers later, use pnputil /add-driver on the exported .inf files.",
        "needs_reboot": False,
        "restore_point_recommended": False,
    },
...
def check_applicability(action_id: str) -> dict:
    ...
    if sys.platform != "win32":
        ...
        if action and action.risk_class == RiskClass.SAFE_READONLY:
            return {
                "applicable": True,
                "reason": "Diagnostic (simulated on non-Windows)",
            }
        return {
            "applicable": False,
            "reason": "Windows-only action on non-Windows platform",
        }
...
def execute_governed_action(
    ...
    after_snapshot = None
    if (
        result.get("status") not in ("submitted",)
        and action.risk_class != RiskClass.SAFE_READONLY
    ):
        try:
    ...
    if action_id.split(".")[0] in _evt_modules and result.get("status") not in (
        "submitted",
    ):
        try:
    ...
    if before_snapshot or after_snapshot:
        _persist_snapshots(
            job_id, session_id, action_id, before_snapshot, after_snapshot
        )
...
def _collect_events_for_action(
    action_id: str, session_id: str, job_id: str
) -> Optional[dict]:
    ...
    try:
        if before:
            SnapshotStore.save(
                ...
                data_json=json.dumps(before, default=str),
            )
        if after:
            SnapshotStore.save(
                job_id=job_id,
                ...
                data_json=json.dumps(after, default=str),
            )
    except Exception as e:
