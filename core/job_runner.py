...
def _apply_command_result(job: "Job", result, duration_ms: int) -> None:
    ...
    job.duration_ms = duration_ms
    # Preserve structured details for the UI (lists of items, counts, etc.).
...
class JobRunner:
    ...
    def __init__(self):
        self._executor = ThreadPoolExecutor(
            max_workers=MAX_WORKERS, thread_name_prefix="cleancpu-job"
        )
    ...
    def _execute_job(self, job: Job):
        ...
        try:
            ...
            logger.info(
                f"Job {job.job_id} {job.status}: {job.action_name} ({job.duration_ms}ms)"
            )
